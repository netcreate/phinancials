/**
 * Sample React Native App
 * https://github.com/facebook/react-native
 * @flow
 */

import React, { Component } from 'react';
import {
  AppRegistry,
  TabBarIOS,
  Image,
} from 'react-native';

const Overview = require('./js/Overview');
const NewExpenseForm = require('./js/NewExpenseForm');
const NewBudgetForm = require('./js/NewBudgetForm');

var base64Icon =
  'data:image/png;base64,';

// './img/test4.png' geeft syntax error en test.jpg niet
/*
 * var ExpenseOverviewIcon =
 * <Image source={require('./img/test.jpg')} />
 */

class Phinancials extends Component {
  state = {
    selectedTab: 'home'
  }

  render() {
    return (
      <TabBarIOS>
        <TabBarIOS.Item
            title="Expense Overview"
            selected={this.state.selectedTab === 'home'}
            icon={{uri: base64Icon, scale: 3}}
            onPress={() => {
              this.setState({
                selectedTab: 'home',
              });
            }}>
          <Overview />
        </TabBarIOS.Item>
        <TabBarIOS.Item
            title="New Form"
            selected={this.state.selectedTab === 'newForm'}
            icon={{uri: base64Icon, scale: 3}}
            onPress={() => {
              this.setState({
                selectedTab: 'newForm',
              });
            }}>
          <NewExpenseForm />
        </TabBarIOS.Item>
        <TabBarIOS.Item
            title="New Budget"
            selected={this.state.selectedTab === 'newBudget'}
            icon={{uri: base64Icon, scale: 3}}
            onPress={() => {
              this.setState({
                selectedTab: 'newBudget',
              });
            }}>
            <NewBudgetForm />
        </TabBarIOS.Item>
      </TabBarIOS>
    );
  }
}

AppRegistry.registerComponent('Phinancials', () => Phinancials);
